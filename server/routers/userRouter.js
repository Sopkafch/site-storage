...
router.post('/login', async(req, res, next) => {
    ...
    const token = jwttoken.sign({id: user.id, email: user.email, role: user.role}, process.env.SECRET_KEY, {expiresIn: '24h'})
    ...
    maxAge: 1000 * 60 * 60 * 24,   // 1 час ||  1000 * 60 * 60 * 24 = 1 сутки
  })
